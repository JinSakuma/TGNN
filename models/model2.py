...
class TGNN(nn.Module):
    ...
    def get_threshold(self, max_frame, thres=0.4):
        """
        thres2を設定する関数
...
#             up_hat = up_
            ...
            if u_pre<self.thres_up and u_<self.thres_up and up_pre>=self.thres_up and up_<self.thres_up:
                if y_pre >= self.thres1:
                    cnt+=1
            ...
            # u推定値が切り替わるタイミング
            if u_pre >= self.thres_up and u_ < self.thres_up:
                # 真値のタイミング
                # u推定値が閾値以下の場合は除外
                if l_c == -1:
                    l_c = 0
