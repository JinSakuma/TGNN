...
def add_hang_over(u, frames=6):
    """
    hang over の処理を追加する関数(baseline, RTG用)
    hang over ... 末尾に無音を加える処理，これによりVADによる発話末検出は
                  実際の発話終了箇所より少し遅くなる
    """

    # u(t) = 0 --> 1 になる瞬間を検知して格納
    ...
    for ii in idxs:
        ...
        # 300ms (6frames 分を 0にする対象のindexに追加)
        target_index += [jj for jj in range(ii, min(ii+6, len(u)))]
    
    # hang over 除去して u(t) = 1 だった部分を 0 に変更
    u[target_index] = 0.
    return u
