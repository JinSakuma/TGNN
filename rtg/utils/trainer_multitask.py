...
def val(net, mode, dataloaders_dict,
    ...
    with torch.no_grad():
        ...
        if resume:
            ...
            print(confusion_matrix(y_true_act, y_pred_act))
            print(confusion_matrix(y_true_act, y_pred_act), file=fo)
            print(accuracy_score(y_true_act, y_pred_act))
            print('応答義務推定Acc: ',accuracy_score(y_true_act, y_pred_act),file=fo)
            fo.close()
