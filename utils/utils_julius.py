...
from utils.utils import u_t_maxcut, make_pack, make_target, MyDataLoader, get_phoneme_id
...
def preprocess(feat, target_type, phase='train'):
    ...
    for i in range(len(feat['feature'])):
        ...
        a_phoneme = feat['feature'][i]['A_phoneme'].tolist()
        pa = get_phoneme_id(a_phoneme)

        b_phoneme = feat['feature'][i]['B_phoneme'].tolist()
        pb = get_phoneme_id(b_phoneme)
...
#             u = u_t_maxcut(u, max_frame=60)
        ...
        batch_list = make_pack(batch_list, pa, idxs, 'phonemeA')
        batch_list = make_pack(batch_list, pb, idxs, 'phonemeB')
